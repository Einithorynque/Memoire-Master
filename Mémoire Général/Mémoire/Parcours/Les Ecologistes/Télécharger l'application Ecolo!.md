---
Description: Le site redirige vers l'AppleStore et GooglePlay pour télécharger l'application Ecolo!
Médias:
  - "[[naviguer-site-footer-lesecologistes.jpg]]"
  - "[[carte-contact-region-locaux-carte.lesecologistes.mp4]]"
Objet d'étude:
  - "[[Les Écologistes]]"
  - "[[Les Écologistes - Carte]]"
  - "[[Écolo! (web)]]"
Parcours:
  - Accueil
  - Communauté
Récurrence: 7.1
Date de création: 2025-09-23T11:02:00
Type:
  - PARCOURS
Réponse au problème: true
Scrolls: 5
Écrans:
  - "1"
  - "0.5"
Complétion: true
---
### Description
Le site redirige vers l'[AppleStore](https://apps.apple.com/us/app/%C3%A9colo/id6468951697) et [GooglePlay](https://play.google.com/store/apps/details?id=org.ecolo.app) pour télécharger l'application Ecolo!

### Affordance

Ces boutons [[Demander|demandent]] de **télécharger** l'application. Puisque qu'ils sont situés à **chaque** fin de page ils [[Encourager|encouragent]] ce téléchargement comme étant la continuité de la navigation du site. Il est transitoire et en voici la conclusion. La récurrence de cet encouragement amplifie celle ci. 

En **redirigeant** vers ces plateformes, [[Les écologistes]] [[Obliger|oblige]] de passer par des plateforme des GAFAM pour télécharger l'application.

Ce compromis peut être questionné à l'onne de la problématique auquel tente de répondre le site : empêcher le réchauffement climatique. En obligeant de passer par leur plateforme, les géants technologiques ==sont légitimés comme unique porte à la technologique.== Ce n'est, d'abord, pas vrai puisque pour télécharger des applications [[Applications#Publier en dehors de store|des alternatives]], souvent open source, existent. Or ces exemples montrent qu'il est possible de concevoir des applications autrement, en dehors d'un modèle de production de la technologie capitaliste dont le monopole des stores est l'effigie. Modèle capitaliste qui est beaucoup critiquer comme étant un des maux à la racine du problème écologique.

Une photo d'un téléphone avec l'application [[Demander|demande]] que celle ci soit téléchargée sur un **téléphone**, donc d'**en posséder un**. Ce signe [[Décourager|décourage]] aussi de naviguer sur le site en utilisant un autre support qu'un téléphone, puisque celui ci invite d'en utiliser un.

Plus tard sur l'AppleStore et le Google Play, les plateformes mais surtout la nature de l'application [[Refuser|refusent]] qu'elle soit téléchargée sur un autre support qu'un téléphone ou, sur l'apple store, qu'un Ipad.

Sachant que "50 % de l’impact carbone du numérique sont liés à la fabrication et au fonctionnement des terminaux", cette incitation qui renforce la dépendance au téléphone entre en conflit avec un objectif de réduction de l'impact carbone de nos sociétés. Surtout quand aucune alternative au simple achat n'est renseigné, mais la présence de marque de Google et Apple si, encourageant l'achat. 
